...
import { Settings, Search, Edit, Trash2, LogOut, X } from 'lucide-react';
...
export default function Sidebar({ chats, activeChatId, onNewChat, onSelectChat, onDeleteChat, onOpenSettings }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside 
        className={`${
          isSidebarOpen ? (isMobile ? 'w-[280px]' : 'w-[220px]') : 'w-0'
        } ${
          isMobile ? 'fixed inset-y-0 left-0 z-50 shadow-2xl' : 'relative'
        } bg-[#171717] transition-all duration-300 flex flex-col h-full overflow-hidden shrink-0 border-r border-white/5`}
      >
        <div className={`p-3 flex flex-col h-full ${isMobile ? 'w-[280px]' : 'w-[220px]'} text-sm text-[#ececec]`}>
          {/* Header for mobile with close button */}
          {isMobile && (
            <div className="flex items-center justify-between mb-4 px-1">
              <span className="font-bold text-yellow-wasp uppercase tracking-widest text-xs">Menu</span>
              <button onClick={closeSidebar} className="p-1 hover:bg-white/10 rounded-full">
                <X size={20} />
              </button>
            </div>
          )}
          ...
          <button 
            onClick={onNewChat}
            className="flex items-center gap-3 w-full p-2.5 rounded-lg hover:bg-[#202020] transition-colors mb-4 font-medium group border border-white/5"
          >
